...
const useEvents = ({
  ...
  const onBlurEvent = e => {
    ...
    if (value.current !== e.target.value) {
      ...
      if (nextDate.isValid()) {
        if (minDate && moment(minDate, formats).isAfter(nextDate) || // before min date
  ...
  return [onRef, onChange, onChangeRaw, onBlurEvent];
};

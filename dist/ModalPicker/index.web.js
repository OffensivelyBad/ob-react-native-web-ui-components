...
const Input = ({
  ...
  const currentStyle = [styles.inputDefaults, themeInputStyle.border, themeInputStyle.background, themeInputStyle.opacity, themeInputStyle.text, styles[type], styles[`${type}Text`], selectStyle, selectTextStyle];
  const {
    color,
    ...otherStyles
  } = StyleSheet.flatten(currentStyle);

  if (fitContent) {
    currentStyle.push({
      width: measureTextWidth(`${value || placeholder || 'aaa'}`) + 30
    });
  } else {
    currentStyle.push({
      width: '100%'
